  ...
  return (
    <HelpdeskLayout title="Help Center" activeSlug={categorySlug} showSearchHero>
      <div className="mb-8 space-y-6">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {quickTasks.map((task) => (
              <Link
                key={task.title}
                href={task.href}
                className={`group border border-outline-variant bg-white p-4 transition-colors hover:border-primary/30 hover:bg-surface-container-low ${
                  task.emphasis ? 'border-primary bg-primary/5' : ''
                }`}
              >
                <div className="flex items-start gap-3">
                  <div className={`flex h-11 w-11 items-center justify-center border ${task.emphasis ? 'border-primary bg-primary text-white' : 'border-outline-variant bg-surface-container-low text-primary'}`}>
        ...
        {selectedCategory && (
          <div className="border border-outline-variant bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <Link
                href={route('helpdesk.index')}
                className="inline-flex items-center justify-center rounded-none border border-outline-variant px-4 py-2 font-label text-xs font-semibold uppercase tracking-[0.18em] text-slate-700 transition-colors hover:border-primary/30 hover:text-primary"
              >
                Clear filter
              </Link>
            </div>
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-500">
                      <Link
                        ...
                        className="rounded-none border border-outline-variant px-3 py-1 font-label text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-700 transition-colors hover:border-primary/30 hover:text-primary"
                      >
                        Open category
                      </Link>
                    </div>
                  ...
                  {parent.children.length > 0 ? (
                    <div className="grid gap-4 xl:grid-cols-2">
                      {parent.children.map((child) => {
                        ...
                        return (
                          <div key={child.id} className="border border-outline-variant bg-surface-container-low p-4">
                            <div className="flex items-start justify-between gap-3">
                              ...
                              <Link
                                ...
                                className="rounded-none border border-outline-variant bg-white px-2.5 py-1 font-label text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-700 transition-colors hover:border-primary/30 hover:text-primary"
                              >
                                Open
                              </Link>
                            </div>
