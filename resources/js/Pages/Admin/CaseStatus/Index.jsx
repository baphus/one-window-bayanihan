...
export default function CaseStatusIndex({ statuses }) {
  ...
  const statusColumns = useMemo(() => [
    ...
    },
    {
      ...
      render: (s) => (
        <>
          <button onClick={() => { setEditingStatus(s); setShowForm(true); }} className="text-indigo-600 hover:text-indigo-900 mr-3 text-sm font-medium">Edit</button>
          {!s.is_system && (
            <button onClick={() => handleDelete(s)} className="text-red-600 hover:text-red-900 text-sm font-medium">Delete</button>
          )}
        </>
      ),
    },
