...
export default function CaseStatusIndex({ statuses }) {
  ...
  const statusColumns = useMemo(() => [
    ...
    },
    {
      ...
      render: (s) => (
        <>
          <button onClick={() => { setEditingStatus(s); setShowForm(true); }} className="min-h-[28px] px-2.5 bg-[#f1f5f9] text-slate-700 hover:bg-slate-200 text-[11px] font-bold rounded-[3px] transition-colors border border-slate-300">Edit</button>
          {!s.is_system && (
            <button onClick={() => handleDelete(s)} className="min-h-[28px] px-2.5 bg-red-50 text-red-600 hover:bg-red-100 text-[11px] font-bold rounded-[3px] transition-colors border border-red-200">Delete</button>
          )}
        </>
      ),
    },
