...
export default function Index() {
  ...
  const [filters, setFilters] = useState({ level: '', search: '', date_from: '', date_to: '', per_page: 20, page: 1 });
  ...
  return (
    <AppLayout title="System Logs">
      ...
      <div data-tour="logs-filters" className="mb-6 grid gap-3 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-4">
        <input type="date" value={filters.date_from} onChange={(e) => updateFilter('date_from', e.target.value)} className="rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-primary focus:outline-none" />

        <input type="date" value={filters.date_to} onChange={(e) => updateFilter('date_to', e.target.value)} className="rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-primary focus:outline-none" />
