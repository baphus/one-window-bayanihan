...
export default function AnonymizedAnalytics() {
    ...
    const totalCases = analytics.cases_by_status?.reduce((acc, curr) => acc + (curr.total || 0), 0) || 0;
    ...
    const categoryChartData = useMemo(() => ({
        labels: analytics.cases_by_category?.map(c => c.name) || [],
        datasets: [{
            label: 'Cases',
            data: analytics.cases_by_category?.map(c => c.count) || [],
            backgroundColor: analytics.cases_by_category?.map(c => c.color) || [],
            borderColor: analytics.cases_by_category?.map(c => c.color) || [],
            borderWidth: 1,
            borderRadius: 4,
        }],
            ...
            <div className="mx-auto max-w-7xl space-y-5 pb-4 px-4 sm:px-6 lg:px-8 pt-6">
                ...
                {isLoading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div className="border border-[#cbd5e1] bg-white p-4">
                            ...
                            <p className="mt-1 text-2xl font-bold text-slate-800">{analytics.referral_stats?.total || 0}</p>
                        </div>
                        <div className="border border-[#cbd5e1] bg-white p-4">
                            <p className="text-[10px] font-extrabold uppercase tracking-[0.11em] text-slate-500">Total Clients</p>
                            <p className="mt-1 text-2xl font-bold text-slate-800">{analytics.total_clients || 0}</p>
                        </div>
                    </div>
                )}
                                ...
                                <table className="w-full border-collapse text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {analytics.cases_by_status?.length > 0 ? (
                                            analytics.cases_by_status.map((item, idx) => (
                                                <tr key={idx}>
                        ...
                        <div className="border border-[#cbd5e1] bg-white p-4">
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full border-collapse text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {analytics.cases_by_service?.length > 0 ? (
                                            analytics.cases_by_service.map((item, idx) => (
                                                <tr key={idx}>
                        ...
                        <div className="border border-[#cbd5e1] bg-white p-4">
                            ...
                            <div className="overflow-y-auto max-h-64 pr-2">
                                <table className="w-full border-collapse text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {analytics.cases_over_time?.length > 0 ? (
                                            analytics.cases_over_time.map((item, idx) => (
                                                <tr key={idx}>
                                ...
                                <div className="mt-4">
                                    ...
                                    <div className="mt-1 flex items-baseline gap-2">
                                        <p className="text-3xl font-extrabold text-slate-800">
                                            {analytics.average_resolution_time?.average_days != null 
                                                ? Number(analytics.average_resolution_time.average_days).toFixed(1) 
                                                : 'N/A'}
                                        </p>
                                        <p className="text-sm font-medium text-slate-500">days</p>
                                    </div>
                                    <p className="mt-2 text-[11px] text-slate-400">
                                        Based on {analytics.average_resolution_time?.resolved_cases || 0} resolved cases.
                                    </p>
                                </div>
                            </div>
                        )}
                        ...
                        {isLoading ? (
                            ...
                            <div className="border border-[#cbd5e1] bg-white p-4 flex-1">
                                ...
                                <div className="grid grid-cols-2 gap-4 mt-2">
                                    <div>
                                        <p className="text-[10px] font-extrabold uppercase tracking-[0.11em] text-slate-500">Total Referrals</p>
                                        <p className="mt-1 text-xl font-bold text-slate-800">{analytics.referral_stats?.total || 0}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-extrabold uppercase tracking-[0.11em] text-slate-500">Avg Per Case</p>
                                        <p className="mt-1 text-xl font-bold text-slate-800">
                                            {analytics.referral_stats?.avg_per_case != null 
                                                ? Number(analytics.referral_stats.avg_per_case).toFixed(1) 
                                                : '0'}
                                        </p>
                                    </div>
                                </div>
                ...
                {isLoading ? (
                    ...
                    <div className="border border-[#cbd5e1] bg-white p-4">
                        ...
                        {analytics.cases_by_category?.length > 0 ? (
                            <div className="h-56">
