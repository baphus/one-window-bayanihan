...
> **Supersedes:** `ARCHITECTURE.md` (v2.0.0). Infrastructure is described by technology and
> capability, never by hosting vendor — see `DEPLOYMENT_GUIDE_v3.0.0.md` §1.
...
PostgreSQL 17 (managed or self-hosted)
    ...
    └─ Queue backend: Redis 7 (jobs/failed_jobs tables in degraded mode)
```
...
| Database | PostgreSQL | 17 | UUID PKs, JSONB, `pgcrypto`, `pg_trgm` |
...
| CAPTCHA | Bot-protection verify API | — | Custom `VerifyTurnstile` middleware; `TURNSTILE_*` keys |
| Error Tracking | Sentry SDK | ^4.8 | `sentry/sentry-laravel`; any compatible ingest endpoint |
| File Storage | S3-compatible object storage | — | `league/flysystem-aws-s3-v3`, path-style endpoint |
| Images | Image CDN SDK | 3.1.3 | Avatar upload/management; optional |
...
| `turnstile` | `VerifyTurnstile` | Login — validates the CAPTCHA token against the configured verify API |
...
```
1. POST /login (email + password + CAPTCHA token)
...
`DEPLOYMENT_GUIDE_v3.0.0.md` §1 runs this topology unchanged.

### Production (any OCI container runtime)

```
HTTPS ingress (TLS termination, X-Forwarded-* → container :8080)
  └─ Application container (≥2 instances, stateless)
       ├─ nginx (reverse proxy, static assets)
       ├─ php-fpm 8.4 (Laravel application)
       └─ supervisord (nginx + php-fpm + queue worker)

Queue workers        same image, `queue:work` (in-container or separate service)
Scheduler            `schedule:run` every minute — exactly once cluster-wide

PostgreSQL 17        primary datastore, TLS, PITR/automated backups
Redis 7              cache, queue broker, OTP store, rate limiters
S3-compatible store  case documents, referral attachments, immutable audit archives
Image CDN            avatar images (optional)
Error-tracking ingest  exception + performance events (optional)
Log collector        container stdout/stderr
```
...
```
docker-compose.yml
  ├─ nginx (1.27-alpine)      reverse proxy → app:9000
  ├─ app (PHP 8.4-fpm)        Laravel + queue worker + scheduler (supervisor)
  ├─ db (postgres:15-alpine)  local database
  └─ redis (7-alpine)         cache, queue, OTP store
```

Local mail is captured with `MAIL_MAILER=log`, or by pointing `MAIL_HOST`/`MAIL_PORT`
at a mail-catcher container of your choice.
...
- **Production:** Incident ID generated, logged, shown to user; forwarded to the error-tracking ingest when configured
...
| Version | Date | Change |
|---|---|---|
| 2.1.0 | 2026-07-27 | Made infrastructure descriptions platform-neutral: §1 system overview, §2 tech stack (database, storage, images, error tracking, CAPTCHA rows), §4/§5 CAPTCHA references, and §9 deployment topology now name technologies and capabilities instead of hosting, managed-database, storage, CDN, or SaaS vendors. §9 production topology expanded with ingress, worker, scheduler, and log-collection roles and cross-referenced to `DEPLOYMENT_GUIDE_v3.0.0.md` §1. Corrected the Docker Compose service list to match `docker-compose.yml` (nginx, app, db, redis — the previous list named a mail service that does not exist and omitted nginx). |
| 2.0.0 | 2026-07-11 | Previous revision (`ARCHITECTURE.md`): vendor-named production topology and tech stack. |

> **Known drift (not changed in this revision):** §12 and §13 still describe the
> queue and cache as database-driver based, while `.env.example` ships
> `QUEUE_CONNECTION=redis` and `CACHE_STORE=redis`. Reconcile these sections
> against the deployed configuration in a separate revision — see
> `REDIS_INTEGRATION_v2.0.0.md`.
