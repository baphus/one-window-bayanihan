...
> **Version:** 1.5.0 | **Updated:** 2026-07-28 | **Supersedes:** `DEPLOYMENT_PRODUCTION_AWS_v1.4.0.md`
> **Operative document.** v1.4.0 remains the reference for how the environment was
> verified and the defect log (its §2, §3, §4, §6). This version adds the
> operations manual — §3 onward — which did not previously exist in writing.
> **Platform-neutral contract:** `docs/DEPLOYMENT_GUIDE_v3.0.0.md`

## 0. What changed from 1.4.0

Nothing about the environment. This version documents **how to operate it**: how to
deploy, how to change configuration, how migrations actually run, how to execute a
one-off command, and — equally important — what is deliberately not possible.

Everything here was verified against the live account, not designed on paper.
...
| Database | Lightsail `bayanihan-production-db` — PostgreSQL 17.10, **not publicly accessible** |
...
| `deploy.yml` | `workflow_call` | Snapshot → deploy → wait READY → `/up` gate → `/api/readyz` gate |
...
# secret
gh secret   set RESEND_KEY              --env production --repo $R --body '<key>'
...
| `MAIL_MAILER` | `log` — OTP/MFA codes go to the container log, so **no user can sign in** | SPF/DKIM/DMARC are published for `owbap.app` and `RESEND_KEY` is set |
...
1. **Mail does not deliver.** `MAIL_MAILER=log`, so OTP and MFA codes are written
   to the container log and no user can complete sign-in. Any code is also readable
   by anyone with console access, which is why the no-real-data rule still applies
   despite `APP_ENV=production`. Needs SPF/DKIM/DMARC for `owbap.app` plus
   `RESEND_KEY`.
