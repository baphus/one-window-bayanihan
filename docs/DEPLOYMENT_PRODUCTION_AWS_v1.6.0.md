...
| 1.7.0 | 2026-07-31 | **Migrated production database from Lightsail managed PostgreSQL to Neon serverless PostgreSQL.** Lightsail managed databases do not support custom extensions (pgvector, etc.), blocking the chatbot embeddings feature. Neon provides pgvector 0.8.0, autoscaling, scale-to-zero, and database branching. `DB_HOST` uses the pooler endpoint (PgBouncer transaction mode) for runtime connection pooling; `NEON_DIRECT_HOST` points to the direct endpoint, used only during migrations (pooler cannot handle multi-statement DDL transactions). The entrypoint temporarily swaps `DB_HOST` to the direct endpoint for `migrate`, then restores the pooler. The old Lightsail managed database (`bayanihan-production-db`) should be snapshotted and decommissioned after verifying production stability. |
