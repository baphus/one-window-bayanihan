...
> **Supersedes:** `TESTING_STRATEGY.md` (v2.0.0)
...
| `SUPABASE_S3_DRIVER` | `local` | Fake local storage. Legacy alias key — the canonical name is `STORAGE_DRIVER`; `config/filesystems.php` still reads the legacy `SUPABASE_S3_*` keys as fallbacks |
| `SUPABASE_S3_ACCESS_KEY` | `fake-access-key` | Legacy alias for `STORAGE_ACCESS_KEY` (also `_SECRET_KEY`, `_REGION`, `_BUCKET`, `_ENDPOINT`) |
...
7. **Storage fakes** — `Storage::fake('object-storage')` for file upload tests (also `'private'`, `'public'`, `'audit-archives'`)
...
| Version | Date | Change |
|---|---|---|
| 2.0.1 | 2026-07-27 | Corrected the storage-fake disk name to `object-storage` (tests use `Storage::fake('object-storage')`; the documented `'supabase'` disk is a legacy alias) and annotated the `phpunit.xml` `SUPABASE_S3_*` overrides as legacy aliases of the canonical `STORAGE_*` keys. No test-strategy change. |
