...
**Version:** v1.0.1
**Date:** 2026-07-11
**Status:** Implemented (autonomous session; assumptions listed below)

## Changelog

| Version | Date | Change |
|---------|------|--------|
| v1.0.1 | 2026-07-11 | Post-review revisions: agency activity card no longer links to the role-gated audit log; `caseTrends` fetched only for case managers; agency payload drops `recentReferrals` (raw models with client PII), `casesByCategory`, `dashboardNotifications`; case-manager payload additionally drops `recentCases`, `casesByProvince`, `agencyBreakdown`, `casesOverTime`, `dashboardNotifications`, `myRecentDrafts`; admin status distribution gains an OTHER bucket with percentages based on total referrals; orphaned `useAdminChartData.js` and `Dashboard/chartConfig.js` removed; triage strip uses hairline `gap-px` dividers. |
